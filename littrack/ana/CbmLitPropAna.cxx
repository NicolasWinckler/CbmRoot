		...
		Option_t* opt)
{
  ...
  for (Int_t iTrack = 0; iTrack < nofTracks; iTrack++) {
      ...
      if (track.GetNofHits() != fNofPlanes) continue;
		...
		std::vector<FairMCPoint*>& points)
{
	...
	if (nofHits != fNofPlanes) return false;
...
{
   fh_srx.resize(fNofPlanes);
   fh_sry.resize(fNofPlanes);

   fh_resx.resize(fNofPlanes);
   fh_resy.resize(fNofPlanes);
   fh_restx.resize(fNofPlanes);
   fh_resty.resize(fNofPlanes);
   fh_resqp.resize(fNofPlanes);

   fh_pullx.resize(fNofPlanes);
   fh_pully.resize(fNofPlanes);
   fh_pulltx.resize(fNofPlanes);
   fh_pullty.resize(fNofPlanes);
   fh_pullqp.resize(fNofPlanes);

   fh_resp.resize(fNofPlanes);
