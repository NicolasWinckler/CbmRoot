...
roc::UdpDataSocket::UdpDataSocket(UdpDevice* dev, dabc::Reference port, int fd, MessageFormat fmt) :
...
void roc::UdpDataSocket::ProcessEvent(const dabc::EventId& evnt)
{
   switch (evnt.GetCode()) {
      case evntSocketRead: {
...
dabc::Buffer& roc::UdpDataSocket::RecvBuffer(unsigned indx) const
{
   ...
   if (indx>=fReadyBuffers)
      throw dabc::Exception(dabc::format("Wrong index %u ready %u in roc::UdpDataSocket::RecvBuffer", indx, fReadyBuffers).c_str());
...
{
   if ((strcmp(name, roc::xmlRocNumber)==0) && fDev) return fDev->rocNumber();
