...
public:
	...
	void SetHits(
	      const std::vector<Hit>& hit)
         {fHits = hit;}

	void SetEllipses(
	      const std::vector<Ellipse>& ellipse)
         {fEllipses = ellipse;}

	const std::vector<Hit>& GetHits()
	      {return fHits;}

	const std::vector<Ellipse>& GetEllipses()
	      {return fEllipses;}

	const std::vector<vector<Double_t> >& GetU()
	      {return fU;}
	
	static Ellipse ParTransform(
	      CbmRichRing* ellipse);
