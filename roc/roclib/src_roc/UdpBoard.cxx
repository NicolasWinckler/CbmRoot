...
int roc::UdpBoard::uploadSDfile(const char* filename, const char* sdfilename)
{
   ...
   *((uint32_t*) buffer) = htonl(bufSize);
   ...
   if (!submitSwPut(ROC_ETH_OVERWRITE_SD_FILE, 1, waittime + 40.)) return -1;

   fRunningUpload = 3;
   fUploadChecksum = 0;

   return int(waittime + 50.);
...
int roc::UdpBoard::uploadBitfile(const char* filename, int position)
{
   ...
   Debug(0, "Start flashing - wait ~180 s.");

   if (!submitSwPut(ROC_ETH_FLASH_KIBFILE_FROM_DDR, position, 220.)) return -1;
