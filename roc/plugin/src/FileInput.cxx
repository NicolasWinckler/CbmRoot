...
bool roc::FileInput::getNextBuffer(void* &buf, unsigned& len, double tmout)
{
   ...
   while (true) {
      ...
      if (fSubev!=0) {
         ...
         if ((fSubev->iProcId == roc::proc_ErrEvent) || (fSubev->iProcId==roc::proc_RocEvent)) {
            // printf("============= Crate = %d ============ sel = %d\n", fSubev->iSubcrate, fSelectRoc);
            ...
            if (!iter.last()) {
               EOUT(("Cannot jump to end of raw buffer"));
               continue;
            }
