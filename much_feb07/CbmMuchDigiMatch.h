...
class CbmMuchDigiMatch : public TObject
{
 ...
 public:
  ...
  CbmMuchDigiMatch(Int_t iPoint, UInt_t iCharge = 0);
  ...
  inline Int_t GetNPoints() const {return fRefIndex.GetSize();}
  
  /** Gets charge from a MC point. Index of the point can 
   * be accessed through GetRefIndex(i). **/
  UInt_t GetCharge(Int_t i) const;

  inline Int_t GetCluster() const {return fClusterIndex;}
  inline void SetCluster(Int_t i) {fClusterIndex = i;}
