...
//   dabc::SetDebugLevel(5);
   ...
   fBnetMode = Cfg(roc::xmlBnetMode, cmd).AsBool(false);
   ...
   if (fSyncScaleDown == 77) {
      DOUT0(("Special mode for FEET readout - take data as is, pack into MBS buffer and store"));
      fSimpleMode = true;
      fSyncScaleDown = -1;
   } else {
   ...
   int numinputs = Cfg(dabc::xmlNumInputs,cmd).AsInt(1);
   ...
   DOUT1(("CombinerModule name:%s numinp = %d SyncScaleDown = %d BnetMode = %s flushtime = %5.1f", GetName(), numinputs, fSyncScaleDown, DBOOL(fBnetMode), flushTime));
   ...
   std::string pname = Cfg(dabc::xmlInputPoolName, cmd).AsStdStr(roc::xmlRocPool);

   fInpPool = CreatePoolHandle(pname.c_str());

   for(unsigned num=0; num < (unsigned) numinputs; num++) {
      CreateInput(FORMAT(("Input%u", num)), fInpPool, Cfg(dabc::xmlInputQueueSize, cmd).AsInt(10));
      Input(num)->SetInpRateMeter(Par("RocData"));

      DOUT2(("Create input%u queue size = %u capacity %u", num, Input(num)->InputQueueSize(), Input(num)->InputQueueCapacity()));

      fInp.push_back(InputRec());
      fInp[num].use = false;
      if (fThrottleAux>=0)
         CreatePar(FORMAT(("Throttle%u", num))).SetRatemeter(false, 3.).SetLimits(0., 100.);
   }

   fOutPool = CreatePoolHandle(Cfg(dabc::xmlOutputPoolName, cmd).AsStdStr(roc::xmlRocPool).c_str());
   ...
   SetInfo(dabc::format("ROC combiner module ready. NumInputs = %d" , numinputs), true);
}
...
void roc::CombinerModule::PoolHandleCleaned(dabc::PoolHandle* pool)
{
   DOUT3(("Cleanup data from pool %p name %s isout %s", pool, pool->GetName(), DBOOL(pool==fOutPool) ));
...
void roc::CombinerModule::ProcessTimerEvent(dabc::Timer* timer)
{
   ...
   if (timer->IsName("FlushTimer")) {
      if (fFlushFlag) FlushOutputBuffer();
      fFlushFlag = true;
   } else

   if (timer->IsName("Get4Timer")) {
      InvokeAllGet4Reset();
   }
}
...
bool roc::CombinerModule::FindNextEvent(unsigned recid)
{
   ...
   if (!rec->isrocid()) {
      ...
      SetInfo(dabc::format("Detect on input %u ROC:%d Kind:%s", recid, rec->rocid, (rec->isudp ? "UDP" : "Optic")), true);
   }
...
//   DOUT5(("FindNextEvent REC:%p ROCID:%d KIND:%d", rec, rec->rocid, rec->isudp));
   ...
   while (port->HasInputBuffer(rec->curr_nbuf)) {

      dabc::Buffer& buf = port->InputBuffer(rec->curr_nbuf);

      if (rec->curr_indx >= buf.GetTotalSize()) {
         if (rec->isprev || rec->isnext) {
            rec->curr_nbuf++;
            if (rec->curr_nbuf == port->InputQueueCapacity()) {
               ...
               rec->isprev = false;
               rec->isnext = false;
               port->SkipInputBuffers(rec->curr_nbuf);
               rec->curr_nbuf = 0;
            }
         } else {
            ...
            if ((fThrottleAux>=0) &&  (data->getAuxChNum()==(unsigned)fThrottleAux)) {
               ...
               if ((rec->last_thottle_tm != 0) && (rec->last_throttle_state != state)) {
                  ...
                  if (rec->last_throttle_state)
                     Par(FORMAT(("Throttle%u", data->getRocNumber()))).SetDouble(dist * 1e-7); // maximum is 100 % can be
               }
...
void roc::CombinerModule::FillSimpleBuffer()
{
   ...
   while (1) {
      ...
      for (unsigned n=0; (n < NumInputs()) && inpbuf.null(); n++)
         if (Input(n)->CanRecv()) {
            ninp = n;
            inpbuf = Input(ninp)->Recv();
            inpptr = inpbuf.GetPointer();
            break;
         }

      if (inpbuf.null()) return;

      if (!CanSendToAllOutputs()) return;

      unsigned rawdata_sz = inpptr.fullsize();

      unsigned needsize = rawdata_sz + sizeof(mbs::EventHeader) + sizeof(mbs::SubeventHeader);
      if (needsize < fBufferSize) needsize = fBufferSize;

      dabc::Buffer outbuf = fOutPool->TakeBuffer(needsize);
      if (outbuf.null()) {
         EOUT(("Cannot get buffer %u from pool", needsize));
         return;
      }
      // outbuf.SetTotalSize(fBufferSize);

      dabc::Pointer outptr = outbuf.GetPointer();

      mbs::EventHeader* evhdr = (mbs::EventHeader*) outptr();

      static int32_t evnt_num(1);

      evhdr->Init(evnt_num++);
      outbuf.Shift(outptr, sizeof(mbs::EventHeader));

      InputRec* rec = &(fInp[ninp]);

      if (!rec->isrocid()) {
         rec->isudp = (Input(ninp)->GetTransportParameter(roc::xmlTransportKind) == kind_UDP);
         rec->rocid = Input(ninp)->GetTransportParameter(roc::xmlRocNumber);
         rec->format = (roc::MessageFormat) Input(ninp)->GetTransportParameter(roc::xmlMsgFormat);

         DOUT0(("Detect on input %u ROC:%d Kind:%s", ninp, rec->rocid, (rec->isudp ? "UDP" : "Optic")));
      }

      mbs::SubeventHeader* subhdr = (mbs::SubeventHeader*) outptr();
      subhdr->Init();
      subhdr->iProcId = roc::proc_RawData;
      subhdr->iSubcrate = rec->rocid;
      subhdr->iControl = rec->format;

      outbuf.Shift(outptr, sizeof(mbs::SubeventHeader));

      unsigned msg_size = roc::Message::RawSize(rec->format);

      // take as much as possible
      if (outptr.fullsize()<rawdata_sz) {
         rawdata_sz = outptr.fullsize() / msg_size * msg_size;
         DOUT1(("Too many raw data %u, take only %u", outptr.fullsize(), rawdata_sz));
      }

      outptr.copyfrom(inpptr, rawdata_sz);
      outbuf.Shift(outptr, rawdata_sz);

      if (rawdata_sz == inpptr.fullsize()) {
         inpptr.reset();
         inpbuf.Release();
      } else {
         inpbuf.Shift(inpptr, rawdata_sz);
      }

      subhdr->SetRawDataSize(rawdata_sz);

      evhdr->SetSubEventsSize(sizeof(mbs::SubeventHeader) + rawdata_sz);

      dabc::BufferSize_t usedsize = outbuf.GetTotalSize() - outptr.fullsize();
      if (usedsize==0) {
         outbuf.Release();
         return;
      }
...
//      DOUT0(("On input %u Skip buffers %u", recid, master_skip));
      ...
      Input(recid)->SkipInputBuffers(can_skip);
...
//      DOUT3(("Try to prepare for event %u", min_evnt));
      ...
      if (fBnetMode || (!skip_evnt && (grand_totalsize > f_outptr.fullsize())))
      ...
      if (!skip_evnt && (fOutBuf.null() || (grand_totalsize > f_outptr.fullsize()))) {
         ...
         f_outptr = fOutBuf.GetPointer();
      }

      if (!skip_evnt) {
         dabc::Pointer old(f_outptr);

//         DOUT5(("Start MBS event at pos %u", fOutBuf.Distance(fOutBuf.GetPointer(), f_outptr)));

//         DOUT0(("Building event %u", min_evnt));

         mbs::EventHeader* evhdr = (mbs::EventHeader*) f_outptr();
         evhdr->Init(min_evnt);
         fOutBuf.Shift(f_outptr, sizeof(mbs::EventHeader));

//         DOUT1(("Fill event %7u  ~size %6u inp0 %ld outsize:%u",
//            min_evnt, totalsize, Input(0)->InputPending(), f_outptr.fullsize()));

//         DOUT1(("Distance to data %u", fOutBuf.Distance(fOutBuf.GetPointer(), f_dataptr)));
         ...
         unsigned filled_sz = FillRawSubeventsBuffer(fOutBuf, f_outptr, extra);
...
//            DOUT0(("Calculated size = %u  filled size = %u", totalsize, filled_sz + sizeof(mbs::EventHeader)));
            ...
            if (fBnetMode) {
               // FIXME: implement bnet
               EOUT(("BNet mode not yet implemented !!!"));

               throw dabc::Exception("BNET mode not yet implemented");
...
//         if (!Output(n)->CanSend())
//            DOUT0(("Cannot send buffer to output %d from %d conn:%s  pend:%d capacity:%d ",
//                  n, NumOutputs(), DBOOL(Output(n)->IsConnected()), Output(n)->OutputPending(), Output(n)->OutputQueueCapacity()));
//      DOUT0(("Cannot send buffer to all output!!!"));
      return false;
   }
...
unsigned roc::CombinerModule::FillRawSubeventsBuffer(dabc::Buffer& outbuf, dabc::Pointer& outptr, roc::MessagesVector* extra)
{
   ...
   for (unsigned ninp = 0; ninp < fInp.size(); ninp++) {
      ...
      while (nbuf<=rec->next_nbuf) {
         dabc::Buffer& inbuf = Input(ninp)->InputBuffer(nbuf);
         if (inbuf.null()) {
            EOUT(("Internal error"));
            return 0;
         }
         ...
         dabc::Pointer ptr = inbuf.GetPointer();
         if (nbuf == rec->next_nbuf) ptr.setfullsize(rec->next_indx + msg_size);
         if (nbuf == rec->prev_nbuf) inbuf.Shift(ptr, rec->prev_indx);
         ...
         if (firstmsg && extra) {
            ...
            inbuf.Shift(ptr, msg_size);
            AddExtraMessagesToSubevent(outbuf, extra, outptr, subeventsize, rec);
         }

         // copy rest of the data
         outptr.copyfrom(ptr, ptr.fullsize());
         outbuf.Shift(outptr, ptr.fullsize());
...
void roc::CombinerModule::AddExtraMessagesToSubevent(dabc::Buffer& buf, roc::MessagesVector* extra, dabc::Pointer& outptr, unsigned& subeventsize, InputRec* rec)
{
   unsigned msg_size = roc::Message::RawSize(rec->format);
...
int roc::CombinerModule::ExecuteCommand(dabc::Command cmd)
{
   if (cmd.IsName("ConfigureInput")) {
      ...
      if (!rec->isrocid()) {
         ...
         rec->format = (roc::MessageFormat) cmd.GetInt("Format", 0);
