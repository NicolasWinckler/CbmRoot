...
void CbmTrdHitRateFastQa::Exec(Option_t * option)
{
  ...
  */
  ...
  for (vector< vector<Int_t> >::size_type j = 0; j < LiSi.size(); j++)
	...
	ScanPlane(LiSi[j][i], c1, h1HitPad);
      /*
...
void CbmTrdHitRateFastQa::ScanPlane(const Int_t moduleAddress, TCanvas*& c1, TH1F*& HitPad)
{
  c1->cd(1);
  //printf("Address%i\n",moduleAddress);
  TVector3 padPos;
...
void CbmTrdHitRateFastQa::HistoInit(TCanvas*& c1, TCanvas*& c2,TH2F*& Layer,TH1F*& HitPad, Double_t ZRangeL, Double_t ZRangeU, Double_t mm2bin)
{
  Char_t name[50];
  Char_t title[50];
