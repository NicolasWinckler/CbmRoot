...
public:
	...
	void StationByPlaneId(
		...
		for(unsigned char i = 0; i < fHits.size(); i++) {
			for(unsigned char j = 0; j < fHits[i].size(); j++) {
	...
	// Arrays with hits
	std::vector<std::vector<std::vector<ScalPixelHitVector> > > fHits;
	...
	friend std::ostream & operator<<(std::ostream &strm, const LitHitData &hitData){
		...
		for(int i = 0; i < hitData.fHits.size(); i++) {
			strm << " station group " << i << std::endl;
			for(int j = 0; j < hitData.fHits[i].size(); j++) {
				strm << "  station " << j << std::endl;
				for(int k = 0; k < hitData.fHits[i][j].size(); k++){
