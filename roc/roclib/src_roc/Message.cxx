...
uint64_t roc::Message::getMsgFullTime(uint32_t epoch) const
{
   switch (getMessageType()) {
      ...
      case MSG_EPOCH2:
         return FullTimeStamp2(getEpoch2Number(), 0) / 40;
      case MSG_GET4:
         return FullTimeStamp2(epoch, getGet4Ts()) / 40;
   }
   return 0;
}
   ...
   if (kind & msg_print_Human) {
      ...
      switch (getMessageType()) {
         ...
         case MSG_EPOCH:
            ...
            os << buf;
            snprintf(buf, sizeof(buf),
                  "EPO @%15.9f Epo:%10d 0x%08x Miss: %3d%c",
         ...
         case MSG_EPOCH2:
            ...
            os << buf;
            snprintf(buf, sizeof(buf),
                  "EPO2 @%15.9f Get4:%2d Epoche2:%10d 0x%08x StampTime:%2d Sync:%x Dataloss:%x", 
						timeInSec, getEpoch2ChipNumber(), getEpoch2Number(), getEpoch2Number(), 
						getEpoch2StampTime(), getEpoch2Sync(), getEpoch2DataLost());
            os << buf << std::endl;
            break;
         case MSG_GET4:
            snprintf(buf, sizeof(buf),
            ...
            os << buf;
            snprintf(buf, sizeof(buf),
                  "Get4 @%15.11f Get4:%2d Chn:%3d Edge:%1d Ts:%7d",
                  timeInSec, getGet4Number(), getGet4ChNum(), getGet4Edge(), getGet4Ts());
            os << buf << std::endl;
            break;
         default:
      ...
      switch (getMessageType()) {
        case MSG_NOP:
         ...
         case MSG_AUX:
            ...
            break;
         case MSG_EPOCH2:
            snprintf(buf, sizeof(buf), "Get4:0x%02x Epoche2:0x%08x StampTime:0x%x Sync:%x Dataloss:%x", 
						getEpoch2ChipNumber(), getEpoch2Number(), getEpoch2StampTime(), getEpoch2Sync(), getEpoch2DataLost());
            break;
         case MSG_GET4:
            snprintf(buf, sizeof(buf), "Get4:0x%02x Chn:%1x Edge:%1x Ts:0x%05x",
                         getGet4Number(), getGet4ChNum(), getGet4Edge(), getGet4Ts());
            break;
         case MSG_SYS: {
            char sysbuf[256];
            ...
            switch (getSysMesType()) {
              ...
              case SYSMSG_NX_PARITY: {
                   ...
                   snprintf(sysbuf, sizeof(sysbuf),"%1d%1d%1d%1d:%02x:%02x:%02x:%02x nX parity error",
            ...
            snprintf(buf, sizeof(buf), "SysType:%2x Nx:%1x Data: %8x : %s",
                    getSysMesType(), getNxNumber(), getSysMesData(), sysbuf);
...
uint32_t roc::Message::RawSize(MessageFormat fmt)
{
   switch (fmt) {
...
bool roc::Message::assign(void* src, MessageFormat fmt)
{
   switch (fmt) {
      case formatEth1:
         convertFromOld(src);
...
bool roc::Message::copyto(void* tgt, MessageFormat fmt)
{
   switch (fmt) {
      case formatEth1:
         convertToOld(tgt);
