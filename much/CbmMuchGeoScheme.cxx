...
void CbmMuchGeoScheme::Init(TObjArray* stations){ 
  ...
  for (map<Long64_t,Int_t>::iterator it=fMapSides.begin();it!=fMapSides.end();it++){
...
// -------------------------------------------------------------------------
CbmMuchLayerSide* CbmMuchGeoScheme::GetLayerSide(Int_t iStation,Int_t iLayer,Bool_t iSide){
...
// -------------------------------------------------------------------------
CbmMuchModule* CbmMuchGeoScheme::GetModule(Int_t iStation, Int_t iLayer, Bool_t iSide, 
					   Int_t iModule){
...
// -------------------------------------------------------------------------
CbmMuchSector* CbmMuchGeoScheme::GetSector(Int_t iStation, Int_t iLayer, Bool_t iSide, 
						  Int_t iModule, Int_t iSector){
...
// -------------------------------------------------------------------------
CbmMuchPad* CbmMuchGeoScheme::GetPad(Int_t iStation, Int_t iLayer, Bool_t iSide, 
				     Int_t iModule, Int_t iSector, Int_t iChannel){
...
void CbmMuchGeoScheme::ResetPads(){
  for (Int_t i = 0;i<fPads.size();i++) fPads[i]->Reset();
}
// -------------------------------------------------------------------------
...
Int_t CbmMuchGeoScheme::GetLayerSideNr(Long64_t detId){
  ...
  if (fMapSides.find(sideId)==fMapSides.end()) Fatal("GetLayerSideNr","Wrong side id");
...
void CbmMuchGeoScheme::ReadGeoFile(const char* geoName){
  ...
  geo.get(b,30);    geo >> fMuchZ1;             geo.getline(b,200);
  geo.get(b,30);    geo >> fAcceptanceTanMin;   geo.getline(b,200);
  geo.get(b,30);    geo >> fAcceptanceTanMax;   geo.getline(b,200);
  geo.get(b,30);    geo >> fNabs;               geo.getline(b,200);
  geo.get(b,30);    geo >> fNst;                geo.getline(b,200);
  geo.get(b,30);    geo >> fModuleDesign;       geo.getline(b,200);
  ...
  geo.get(b,30);   for (i=0;i<fNabs;i++) geo >> fAbsorberZ1 [i];  geo.getline(b,200);
  geo.get(b,30);   for (i=0;i<fNabs;i++) geo >> fAbsorberLz [i];  geo.getline(b,200);
  geo.get(b,30);   for (i=0;i<fNabs;i++) geo >> fAbsorberMat[i];  geo.getline(b,200);

  geo.getline(b,200); //
  geo.getline(b,200); // # Station specification
  geo.get(b,30);   for (i=0;i<fNst;i++) geo >> fStationZ0[i];     geo.getline(b,200);
  geo.get(b,30);   for (i=0;i<fNst;i++) geo >> fNlayers  [i];     geo.getline(b,200);
  geo.get(b,30);   for (i=0;i<fNst;i++) geo >> fDetType  [i];     geo.getline(b,200);
  geo.get(b,30);   for (i=0;i<fNst;i++) geo >> fLayersDz [i];     geo.getline(b,200);
  geo.get(b,30);   for (i=0;i<fNst;i++) geo >> fSupportLz[i];     geo.getline(b,200);
  geo.get(b,30);   for (i=0;i<fNst;i++) geo >> fSigmaXmin[i];     geo.getline(b,200);
  geo.get(b,30);   for (i=0;i<fNst;i++) geo >> fSigmaYmin[i];     geo.getline(b,200);
  geo.get(b,30);   for (i=0;i<fNst;i++) geo >> fSigmaXmax[i];     geo.getline(b,200);
  geo.get(b,30);   for (i=0;i<fNst;i++) geo >> fSigmaYmax[i];     geo.getline(b,200);
  geo.get(b,30);   for (i=0;i<fNst;i++) geo >> fOccupancyMax[i];  geo.getline(b,200);

  geo.getline(b,200); //
  geo.getline(b,200); // # GEM module specification
  geo.get(b,30);   geo >> fActiveLx;    geo.getline(b,200);
  geo.get(b,30);   geo >> fActiveLy;    geo.getline(b,200);
  geo.get(b,30);   geo >> fActiveLz;    geo.getline(b,200);
  geo.get(b,30);   geo >> fSpacerLx;    geo.getline(b,200);
  geo.get(b,30);   geo >> fSpacerLy;    geo.getline(b,200);
  geo.get(b,30);   geo >> fOverlapY;    geo.getline(b,200);
  geo.close();
}
// -------------------------------------------------------------------------
...
void CbmMuchGeoScheme::Print(){
  ...
  printf("  Absorber Zin position [cm]  :"); for (i=0;i<fNabs;i++) printf("%7.1f",fAbsorberZ1[i]);     printf("\n");
  printf("  Absorber thickness [cm]     :"); for (i=0;i<fNabs;i++) printf("%7.1f",fAbsorberLz[i]);     printf("\n");
  printf("  Absorber material           :"); for (i=0;i<fNabs;i++) printf("      %c",fAbsorberMat[i]); printf("\n");
  printf("\n");
  printf("Station specification:\n");
  printf("  Station Zceneter [cm]       :"); for (i=0;i<fNst;i++) printf("%7.1f",fStationZ0[i]);     printf("\n");
  printf("  Number of layers            :"); for (i=0;i<fNst;i++) printf("%7i"  ,fNlayers  [i]);     printf("\n");
  printf("  Detector type               :"); for (i=0;i<fNst;i++) printf("%7i"  ,fDetType  [i]);     printf("\n");
  printf("  Distance between layers [cm]:"); for (i=0;i<fNst;i++) printf("%7.1f",fLayersDz [i]);     printf("\n");
  printf("  Support thickness [cm]      :"); for (i=0;i<fNst;i++) printf("%7.1f",fSupportLz[i]);     printf("\n");
  printf("  Sigma X minimum [cm]        :"); for (i=0;i<fNst;i++) printf("%7.2f",fSigmaXmin[i]);     printf("\n");
  printf("  Sigma Y minimum [cm]        :"); for (i=0;i<fNst;i++) printf("%7.2f",fSigmaYmin[i]);     printf("\n");
  printf("  Sigma X maximum [cm]        :"); for (i=0;i<fNst;i++) printf("%7.2f",fSigmaXmax[i]);     printf("\n");
  printf("  Sigma Y maximum [cm]        :"); for (i=0;i<fNst;i++) printf("%7.2f",fSigmaYmax[i]);     printf("\n");
  printf("  Maximum occupancy           :"); for (i=0;i<fNst;i++) printf("%7.2f",fOccupancyMax[i]);  printf("\n");
...
void CbmMuchGeoScheme::CreateMuchCave(){
  ...
  Double_t muchZ2 = fMuchZ1
                  + fStationZ0[fNst-1]
                  + fLayersDz[fNst-1]*(fNlayers[fNst-1]-1)/2.
                  + (fSupportLz[fNst-1]+fActiveLz)/2.
                  + 3.;
...
void CbmMuchGeoScheme::CreateStations(){
  ...
  for (Int_t st=0;st<fNst;st++){
    ...
    Double_t stDz = ((fNlayers[st]-1)*fLayersDz[st]+fSupportLz[st]+fActiveLz)/2.+1;
      ...
      if (fModuleDesign) {
        ...
        for (Int_t ix=1;ix<=nx;ix++){
          ...
          for (Int_t iy=-ny;iy<=ny;iy+=2){
            ...
            if (pow(mX-fActiveLx/2.,2)+pow(mY-fActiveLy/2.,2)>rmax*rmax &&
                pow(mX-fActiveLx/2.,2)+pow(mY+fActiveLy/2.,2)>rmax*rmax) continue;
            // Create module with positive x
            TVector3 pos = TVector3(mX,mY,mZ+layerGlobalZ0);
            side->AddModule(new CbmMuchModule(st,l,isBack,m,pos,size,rHole));
            m++;
        ...
        layer->SetSupportDy((ny+1)*(fActiveLy-fOverlapY)/2.+fOverlapY/2.+fSpacerLy);
...
// -------------------------------------------------------------------------
Int_t CbmMuchGeoScheme::Intersect(Float_t x, Float_t y, 
                                  Float_t dx, Float_t dy, Float_t r){
  Float_t x1 = x-dx;
  Float_t x2 = x+dx;
  Float_t y1 = y-dy;
  Float_t y2 = y+dy;
