...
{
  fHistoMap.clear();
  fHistoMap["MC_EventCounter"] = new TH1I("MC_EventCounter","MC_EventCounter",100,0,100);
  ...
  for (Int_t i = 0; i < 3; i++) {
    fHistoMap["PairAllVertex_" + name[i]] = new TH1I("PairAllVertex_" + name[i], "all e^{+} e^{-} pair vertex " + name[i], 1000, low[i], high[i]);
    fHistoMap["PairGammaVertex_" + name[i]] = new TH1I("PairGammaVertex_" + name[i], "e^{+} e^{-} pair from #gamma vertex " + name[i], 1000, low[i], high[i]);
    fHistoMap["PairPi0Vertex_" + name[i]] = new TH1I("PairPi0Vertex_" + name[i], "e^{+} e^{-} pair from #pi^{0} vertex " + name[i], 1000, low[i], high[i]);

    fHistoMap["pi02GammaVertex_" + name[i]] = new TH1I("pi02GammaVertex_" + name[i], "decay vertex of #pi^{0} to 2#gamma or e^{+}e^{-}#gamma" + name[i], 1000, low[i], high[i]);
    fHistoMap["gamma2PairVertex_" + name[i]] = new TH1I("gamma2PairVertex_" + name[i], "decay vertex of #gamma to e^{+} e^{-} pair " + name[i], 1000, low[i], high[i]);

    NiceHisto1(fHistoMap["PairAllVertex_" + name[i]],1,0,0,TString(name[i]+"-position [cm]"),"");
    NiceHisto1(fHistoMap["PairGammaVertex_" + name[i]],2,0,0,TString(name[i]+"-position [cm]"),"");
    NiceHisto1(fHistoMap["PairPi0Vertex_" + name[i]],3,0,0,TString(name[i]+"-position [cm]"),"");

    NiceHisto1(fHistoMap["pi02GammaVertex_" + name[i]],1,0,0,TString(name[i]+"-position [cm]"),"");
    NiceHisto1(fHistoMap["gamma2PairVertex_" + name[i]],2,0,0,TString(name[i]+"-position [cm]"),"");
  }
  /*
  ...
  */
  ...
  fHistoMap["InvMassSpectrumGammaEPPairsOA"] = new TH2I("InvMassSpectrumGammaEPPairsOA","Invariant mass spectrum from true MC-e^{+}e^{-} pairs",200,0,2,180,0,180); // new
  ...
  fHistoMap["InvMassSpectrumGammaEPPairsVD"] = new TH2I("InvMassSpectrumGammaEPPairsVD","Invariant mass spectrum from true MC-e^{+}e^{-} pairs",200,0,2,1000,0,1e-10);
}
...
void CbmTrdPhotonAnalysis::InitHistos()
{
  ...
  NiceHisto2((TH2*)fHistoMap["InvMassSpectrumGammaEPPairsInTargetOA"],1,1,1,"Invariant mass [GeV/c^{2}]","#gamma pair opening angle #theta [#circ]",""); // new
  NiceHisto2((TH2*)fHistoMap["InvMassSpectrumGammaEPPairsInMagnetOA"],1,1,1,"Invariant mass [GeV/c^{2}]","#gamma pair opening angle #theta [#circ]",""); // new
