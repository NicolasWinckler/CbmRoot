...
InitStatus CbmTrdDigitizerPRF::Init()
{
   ...
   if (fPoints == NULL) LOG(FATAL) << "CbmTrdDigitizerPRF::Init No TrdPoints array" << FairLogger::endl;
   ...
   ioman->Register("TrdDigi", "TRD", fDigis, kTRUE);

   fDigiMatches = new TClonesArray("CbmMatch", 100);
   ioman->Register("TrdDigiMatch", "TRD", fDigiMatches, kTRUE);
...
void CbmTrdDigitizerPRF::Exec(Option_t * option)
{
   ...
   for (Int_t iPoint = 0; iPoint < nofPoints ; iPoint++) {
      ...
      // Add TR energy loss for electrons and positrons
      Double_t ELoss = 0.0;
      Double_t ELossTR = 0.0;
      Double_t ELossdEdX = 0.0;
      if(TMath::Abs(track->GetPdgCode()) == 11) {
         nofElectrons++;
         if (fRadiators->LatticeHit(point)) {  // electron has passed lattice grid (or frame material) before reaching the gas volume -> TR-photons have been absorbed by the lattice grid
            nofLatticeHits++;
            ELossTR = 0.0;
