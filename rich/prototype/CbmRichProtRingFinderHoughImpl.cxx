...
void CbmRichProtRingFinderHoughImpl::DoFind()
{
	if (fData.size() > kMAX_NOF_HITS) {
		cout<< "-E- CbmRichProtRingFinderHoughImpl::DoFind: Number of hits is more than "<< kMAX_NOF_HITS << endl;
		return ;
	}
	...
	std::sort(fData.begin(), fData.end(), CbmRichProtHoughHitCmpUp());
	HoughTransformReconstruction();
	RingSelection();
	fData.clear();
}

void CbmRichProtRingFinderHoughImpl::SetParameters( Int_t nofParts,
...
void CbmRichProtRingFinderHoughImpl::SetParameters(TString geometry)
{
    if (geometry != "compact" && geometry != "large"){
        geometry = "compact";
...
void CbmRichProtRingFinderHoughImpl::DefineLocalAreaAndHits(Float_t x0, Float_t y0,
		Int_t *indmin, Int_t *indmax)
{
    CbmRichProtHoughHit mpnt;
    std::vector<CbmRichProtHoughHit>::iterator itmin, itmax;

	//find all hits which are in the corridor
	mpnt.fHit.fX = x0 - 0.36f*fMaxDistance;
	itmin = std::lower_bound(fData.begin(), fData.end(), mpnt, CbmRichProtHoughHitCmpUp());

	mpnt.fHit.fX = x0 + fMaxDistance;
	itmax = std::lower_bound(fData.begin(), fData.end(), mpnt, CbmRichProtHoughHitCmpUp()) - 1;
...
void CbmRichProtRingFinderHoughImpl::HoughTransform(unsigned short int indmin, unsigned short int indmax)
{
...
void CbmRichProtRingFinderHoughImpl::HoughTransformGroup(unsigned short int indmin,
		unsigned short int indmax, Int_t iPart)
{
    ...
    register vector<CbmRichProtHoughHit> data;
    if (iPart == 0){
    	data = fDataPart1;
	...
	typedef vector<CbmRichProtHoughHit>::iterator iH;
...
void CbmRichProtRingFinderHoughImpl::FindPeak(Int_t indmin, Int_t indmax)
{
...
void CbmRichProtRingFinderHoughImpl::RemoveHitsAroundRing(Int_t indmin, Int_t indmax,
...
void CbmRichProtRingFinderHoughImpl::RingSelection()
{
	...
	std::sort(fFoundRings.begin(), fFoundRings.end(), CbmRichProtRingComparatorMore());
...
void CbmRichProtRingFinderHoughImpl::ReAssingSharedHits(Int_t ringInd1, Int_t ringInd2)
{
...
void CbmRichProtRingFinderHoughImpl::ReAssingSharedHitsEllipse(Int_t ringInd1, Int_t ringInd2)
{
...
Int_t CbmRichProtRingFinderHoughImpl::GetHitIndex(UShort_t hitInd)
{
...
Bool_t CbmRichProtRingFinderHoughImpl::AreRingsCloseEnough(CbmRichRingLight* ring1,
		CbmRichRingLight* ring2)
{
...
void CbmRichProtRingFinderHoughImpl::InitHist()
{
	fHist[0]=0;
	fHist[1]=0;
	fHist[2]=0;
...
void CbmRichProtRingFinderHoughImpl::FuzzyKE(TClonesArray* rHitArray)
{
/*	Int_t nofRings = fFoundRings.size();
...
void CbmRichProtRingFinderHoughImpl::CalculateRingParameters(Float_t x2y2[],
                             Float_t rx[],
                             Float_t ry[],
                             Float_t x0,
                             Float_t y0,
...
void CbmRichProtRingFinderHoughImpl::CalculateRingParametersOld(Float_t x[],
			Float_t y[],
			Float_t *xc,
			Float_t *yc,
			Float_t *r)
...
void CbmRichProtRingFinderHoughImpl::RemoveHitsAroundEllipse(Int_t indmin, Int_t indmax, CbmRichRing * ring)
{
