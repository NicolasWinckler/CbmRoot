...
int Tiltandshift(int Station_number, int Layer_number, float Layer_thickness, float Layer_pitch, float Position_Station[][4], int i) {
  ...
  float station_z_offset[3] = { 0., 1500., 4500. };   // distance in z between 1st layer of stations 
  float z_offset = station_z_offset[Station_number-1] + Layer_thickness/2;
  if ((Station_number == 2) && (Layer_number >= 2))
    z_offset += 500.;
...
int main(void)
{
 ...
 //-----------Files------------------------------------------------
  ...
  string geoname  = path + "trd_v12x.geo" ;
...
//        fprintf(geofile,"//*********************************\n");
        ...
        if ( istation+1 == 1 ) {
          if (ilayer+1 >= 3) printf("skipping TRD1 Layer %d\n", ilayer+1);
	  else if ((TrdModules1(istation+1, ilayer , Frame_width, Layer_thickness, Layer_pitch, Chamber_number_Station1, Position_Station1, Detector_size_x, Detector_size_y ))) {
