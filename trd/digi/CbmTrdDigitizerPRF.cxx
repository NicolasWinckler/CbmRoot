...
void CbmTrdDigitizerPRF::Exec(Option_t * option)
{
  ...
  for (Int_t j = 0; j < nEntries ; j++ ){
    ...
    //if (fDebug)
    //if (CbmTrdAddress::GetModuleId(fModuleInfo->GetModuleAddress()) != 5) continue;
    ...
    //if (fDebug)
    //if (CbmTrdAddress::GetModuleId(fModuleInfo->GetModuleAddress()) == 5) break;
  }
...
/*
  // --------------------------------------------------------------------
Double_t CbmTrdDigitizerPRF::GetGlobalDisplacement(Int_t nPad, Double_t pSize, Double_t dis){
  Int_t sign = 1;
    if (nPad < 0)
      sign = -1;

//  return nPad * pSize + sign * dis;
//
//  return nPad * pSize - dis;  // neg
  return nPad * pSize - dis;  // pos
}
  // --------------------------------------------------------------------
  Double_t CbmTrdDigitizerPRF::GetMirroredDisplacement(Double_t dis, Double_t pSize){
    return dis;
    // we do not nee the rest ...
    Int_t sign = 1;
    if (dis < 0)
      sign = -1;

    Int_t nPads = Int_t(dis / pSize + 0.5 * sign); // trunk of number of pads 

    printf("dis: %g, pSize: %g, sign: %d\n", dis, pSize, sign);
    printf("nPads: %d\n", nPads);
    printf("doff : %g\n", -(dis - nPads * pSize));
    printf("returned: %g\n", -(dis - nPads * pSize) + nPads * pSize);

    return -(dis - nPads * pSize) + nPads * pSize;
  }
*/
  // --------------------------------------------------------------------
