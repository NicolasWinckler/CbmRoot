...
inline void L1ExtrapolateYC11Line( L1TrackPar &T, fvec z_out )
{
  const fvec dz = (z_out - T.z);
  T.y += T.ty*dz;
  T.C11 += dz * ( 2*T.C31 + dz * T.C33 );
}

inline void L1ExtrapolateXC00Line( L1TrackPar &T, fvec z_out )
{
  const fvec dz = (z_out - T.z);
  T.x += T.tx*dz;
  T.C00 += dz * ( 2*T.C20 + dz * T.C22 );
}
