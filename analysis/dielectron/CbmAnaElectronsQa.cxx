...
CbmAnaElectronsQa::CbmAnaElectronsQa(const char *name, const char *title, Int_t verbose)
  :FairTask(name)
{
	fEventNum = 1;
...
Bool_t CbmAnaElectronsQa::SetParameters()
{
	...
	if (fUseRichAnn){
		if (fRichGeoType != "compact" && fRichGeoType != "large"){
			...
			cout << "-E- CbmAnaElectronsQa::SetParameters UNKNOWN geometry,  " <<
...
void CbmAnaElectronsQa::AccTracks()
{
	...
	for (it=fRingMap.begin(); it!=fRingMap.end(); it++) {
		...
		if (!track) {
			cout << "-E- CbmAnaElectronsQa::AccRichRings() no track"<< it->first<<endl;
			continue;
		}
...
void CbmAnaElectronsQa::FoundRichRings()
{
...
void CbmAnaElectronsQa::RingTrackMatch()
{
    Int_t nMatches = fRichRingMatches->GetEntriesFast();
...
void CbmAnaElectronsQa::GlobalTracksMatchEff()
{
...
void CbmAnaElectronsQa::GlobalTracksElIdEff()
{
	Int_t nGlTracks = fGlobalTracks->GetEntriesFast();
...
Bool_t CbmAnaElectronsQa::IsTrdElectron(CbmTrdTrack* trdTrk)
{
...
void CbmAnaElectronsQa::DiffElandPi()
{
	Int_t nGlTracks = fGlobalTracks->GetEntriesFast();
...
void CbmAnaElectronsQa::SaveToImage()
{
