...
int nxyter::NxI2c::probe()
{
  ...
  for (int i=0; i<3; i++) {
    ...
    if (rc) return Board::operErrBuild(rc, 3+i);
  }

  // read and check pattern
  for (int i=0; i<3; i++) {
    rc = getRegister(regs[i], back[i]);
    if (rc) return Board::operErrBuild(rc, 6+i);
    if (back[i] != patt[i]) 
      return Board::operErrBuild(Board::kOperVerifyErr, 6+i);
  }

  // restore registers
  for (int i=0; i<3; i++) {
    rc = setRegister(regs[i], save[i], true);
    if (rc) return Board::operErrBuild(rc, 9+i);
  }
...
int nxyter::NxI2c::setRegMask(uint8_t *val)
{
...
int nxyter::NxI2c::setRegCore(uint8_t *val)
{
  int rc;
...
int nxyter::NxI2c::setRegTrim(uint8_t *val)
{
  int rc;
...
int nxyter::NxI2c::getRegMask(uint8_t *val)
{
  ...
  rc = getRegister(kNxRegConfig0, conf0);
  if (rc) return rc;
  if (conf0 & kNxC0TestTrigEnable) {        // temporay test trigger disable
