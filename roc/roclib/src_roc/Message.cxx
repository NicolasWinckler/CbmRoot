   ...
   if (kind & msg_print_Human) {
      ...
      switch (getMessageType()) {
         ...
         case MSG_EPOCH2:
            ...
            os << buf;
            snprintf(buf, sizeof(buf),
                  "EPO2 @%17.11f Get4:%2d Epoche2:%10u 0x%08x StampTime:%2d Sync:%x Dataloss:%x Epochloss:%x", 
                        timeInSec, getEpoch2ChipNumber(), getEpoch2Number(), getEpoch2Number(), 
                        getEpoch2StampTime(), getEpoch2Sync(), getEpoch2DataLost(), getEpoch2EpochLost());
            os << buf << std::endl;
            break;
         case MSG_GET4:
            snprintf(buf, sizeof(buf),
      ...
      switch (getMessageType()) {
        case MSG_NOP:
         ...
         case MSG_AUX:
            ...
            break;
         case MSG_EPOCH2:
            snprintf(buf, sizeof(buf), "Get4:0x%02x Epoche2:0x%08x StampTime:0x%x Sync:%x Dataloss:%x Epochloss:%x", 
                     getEpoch2ChipNumber(), getEpoch2Number(), getEpoch2StampTime(), getEpoch2Sync(), getEpoch2DataLost(), getEpoch2EpochLost());
            break;
         case MSG_GET4:
