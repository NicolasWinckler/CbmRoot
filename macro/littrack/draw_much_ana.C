...
draw_much_ana()
{
  ...
  TString outDir = "./lit_before_update/";
  ...
  TFile *file1 = new TFile(dir + beam + "/" 
   ...
   for (int i = 0; i < fNofLayers; i++) {
      ...
      hist(file1, c_srx, i, "fh_srx%i", sigma_srx[i]);
      hist(file1, c_sry, i, "fh_sry%i", sigma_sry[i]);
      
      hist(file1, c_resx, i, "fh_resx%i", sigma_resx[i]);
      hist(file1, c_resy, i, "fh_resy%i", sigma_resy[i]);
      hist(file1, c_restx, i, "fh_restx%i", sigma_restx[i]);
      hist(file1, c_resty, i, "fh_resty%i", sigma_resty[i]);
      hist(file1, c_resqp, i, "fh_resqp%i", sigma_resqp[i]);
      
      hist(file1, c_pullx, i, "fh_pullx%i", sigma_pullx[i]);
      hist(file1, c_pully, i, "fh_pully%i", sigma_pully[i]);
      hist(file1, c_pulltx, i, "fh_pulltx%i", sigma_pulltx[i]);
      hist(file1, c_pullty, i, "fh_pullty%i", sigma_pullty[i]);
      hist(file1, c_pullqp, i, "fh_pullqp%i", sigma_pullqp[i]);
      
      hist(file1, c_resp, i, "fh_resp%i", sigma_resp[i]);
        
   }

   
   
   hist2(file1, c_last, 0, "fh_resx_last", sigma_last[0]);
   hist2(file1, c_last, 1, "fh_resy_last", sigma_last[1]);
   hist2(file1, c_last, 2, "fh_restx_last", sigma_last[2]);
   hist2(file1, c_last, 3, "fh_resty_last", sigma_last[3]);
   hist2(file1, c_last, 4, "fh_resqp_last", sigma_last[4]);
   hist2(file1, c_last, 5, "fh_pullx_last", sigma_last[5]);
   hist2(file1, c_last, 6, "fh_pully_last", sigma_last[6]);
   hist2(file1, c_last, 7, "fh_pulltx_last", sigma_last[7]);
   hist2(file1, c_last, 8, "fh_pullty_last", sigma_last[8]);
   hist2(file1, c_last, 9, "fh_pullqp_last", sigma_last[9]);
   hist2(file1, c_last, 10, "fh_resp_last", sigma_last[10]);  
   
   hist2(file1, c_first, 0, "fh_resx_first", sigma_first[0]);
   hist2(file1, c_first, 1, "fh_resy_first", sigma_first[1]);
   hist2(file1, c_first, 2, "fh_restx_first", sigma_first[2]);
   hist2(file1, c_first, 3, "fh_resty_first", sigma_first[3]);
   hist2(file1, c_first, 4, "fh_resqp_first", sigma_first[4]);
   hist2(file1, c_first, 5, "fh_pullx_first", sigma_first[5]);
   hist2(file1, c_first, 6, "fh_pully_first", sigma_first[6]);
   hist2(file1, c_first, 7, "fh_pulltx_first", sigma_first[7]);
   hist2(file1, c_first, 8, "fh_pullty_first", sigma_first[8]);
   hist2(file1, c_first, 9, "fh_pullqp_first", sigma_first[9]);
   hist2(file1, c_first, 10, "fh_resp_first", sigma_first[10]); 
   ...
   fout << "------------search region sigma---------------" << endl;
   for (int i = 0; i < fNofLayers; i++) {
      fout << "srx[" << i << "]=" << sigma_srx[i] << ";   "
           << "sry[" << i << "]=" << sigma_sry[i] << ";" << endl;
   }
   
   fout << "------------sigma resolution---------------" << endl;
   for (int i = 0; i < fNofLayers; i++) {
      fout << "resx[" << i << "]=" << sigma_resx[i] << ";    "
           << "resy[" << i << "]=" << sigma_resy[i] << ";    " 
           << "restx[" << i << "]=" << sigma_restx[i] << ";    "
           << "resty[" << i << "]=" << sigma_resty[i] << ";    " 
           << "resqp[" << i << "]=" << sigma_resqp[i] << ";    " << endl;
   }
   
   fout << "------------sigma pulls---------------" << endl;
   for (int i = 0; i < fNofLayers; i++) {
      fout << "pullx[" << i << "]=" << sigma_pullx[i] << ";    "
           << "pully[" << i << "]=" << sigma_pully[i] << ";    " 
           << "pulltx[" << i << "]=" << sigma_pulltx[i] << ";    "
           << "pullty[" << i << "]=" << sigma_pullty[i] << ";    " 
           << "pullqp[" << i << "]=" << sigma_pullqp[i] << ";    " << endl; 
   }  
   
   fout << "------------momentum residual---------------" << endl;
   for (int i = 0; i < fNofLayers; i++) {
      fout << "resp[" << i << "]=" << sigma_resp[i] << " " << endl; 
   }  
...
void hist(TFile *file1, TCanvas *c1, int i, const char* name, double &sigma) {
...
void hist2(TFile *file1, TCanvas *c1, int i, const char* name, double &sigma) {
